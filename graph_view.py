...
class FamilyGraphWindow(QWidget):
    ...
    def _theme_embedded_editor_web(self) -> None:
        ...
        js = (
            ...
            "}"
            # Hide AJpC addon toolbar shortcuts that are not needed in graph-embedded editor.
            "var targets={'list':1,'graph':1,'deps':1};"
            "var scopes=["
            " document.querySelector('.toolbar'),"
            " document.getElementById('topbuts'),"
            " document.querySelector('.edit_tools'),"
            ...
            " var scope=scopes[si];"
            " if(!scope) continue;"
            " var btns=scope.querySelectorAll('button,a,[role=\"button\"],.hitem,.linkb,.btn,span');"
            " for(var bi=0;bi<btns.length;bi++){"
            "   var b=btns[bi];"
            "   if(!b) continue;"
            "   var txt=((b.textContent||'')+'').trim().toLowerCase();"
            "   var title=((b.getAttribute&&b.getAttribute('title'))||'').trim().toLowerCase();"
            "   var hit=(targets[txt]||targets[title]);"
            "   if(!hit) continue;"
            "   var host=(b.closest&&b.closest('button,a,[role=\"button\"],.hitem,.linkb,.btn'))||b;"
