...
(function () {
  ...
  function patchSolidFragmentShader(source) {
    ...
    var injection = [
      ...
      "    float ajpcSpacing = max(ajpcHalfThickness * 8.0, 18.0);",
      "    float ajpcRadius = max(ajpcHalfThickness * 1.15, 1.8);",
      "    float ajpcTravel = u_time * ajpcSpeed;",
      "    float ajpcLocalA = mod(ajpcAlongWorld - ajpcTravel, ajpcSpacing) - (ajpcSpacing * 0.5);",
      "    float ajpcLocalB = mod(ajpcAlongWorld - ajpcTravel - (ajpcSpacing * 0.5), ajpcSpacing) - (ajpcSpacing * 0.5);",
      "    float ajpcDistA = sqrt((ajpcLocalA * ajpcLocalA) + (ajpcRadial * ajpcRadial));",
      "    float ajpcDistB = sqrt((ajpcLocalB * ajpcLocalB) + (ajpcRadial * ajpcRadial));",
      "    float ajpcCoreA = 1.0 - smoothstep(ajpcRadius * 0.25, ajpcRadius * 0.95, ajpcDistA);",
      "    float ajpcCoreB = 1.0 - smoothstep(ajpcRadius * 0.25, ajpcRadius * 0.95, ajpcDistB);",
      "    float ajpcGlowA = 1.0 - smoothstep(ajpcRadius * 0.95, ajpcRadius * 2.3, ajpcDistA);",
      "    float ajpcGlowB = 1.0 - smoothstep(ajpcRadius * 0.95, ajpcRadius * 2.3, ajpcDistB);",
      "    float ajpcPhoton = clamp(max(max(ajpcCoreA, ajpcCoreB), 0.55 * max(ajpcGlowA, ajpcGlowB)), 0.0, 1.0) * ajpcFlowGate;",
      "    float ajpcFlowRgbMul = 1.0 + (0.90 * ajpcPhoton);",
      "    float ajpcFlowAlphaMul = 1.0 + (0.75 * ajpcPhoton);",
      "    float ajpcAlpha = clamp(v_color.a * ajpcMask * ajpcAlphaMul * ajpcFlowAlphaMul, 0.0, 1.0);",
      "    vec3 ajpcRgb = (v_color.rgb * ajpcRgbMul) * ajpcFlowRgbMul;",
      "    ajpcRgb = mix(ajpcRgb, vec3(0.92, 0.97, 1.0), 0.42 * ajpcPhoton);",
