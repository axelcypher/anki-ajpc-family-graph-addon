...
(function () {
  ...
  function patchSolidFragmentShader(source) {
    ...
    var injection = [
      ...
      "    float ajpcSpacing = max(ajpcHalfThickness * 18.0, 36.0);",
      "    float ajpcRadius = max(ajpcHalfThickness * 2.2, 3.8);",
      ...
      "    float ajpcCore = 1.0 - smoothstep(ajpcRadius * 0.22, ajpcRadius * 0.92, ajpcDist);",
      "    float ajpcGlow = 1.0 - smoothstep(ajpcRadius * 0.92, ajpcRadius * 2.5, ajpcDist);",
      "    float ajpcPhoton = clamp(max(ajpcCore, 0.55 * ajpcGlow), 0.0, 1.0) * ajpcFlowGate;",
