...
- Legacy editor-only hover tokens `--button-bg-hover` and `--btn-border-hover` were consolidated to the shared interactive control tokens (`--control-bg-interactive`, `--control-border-interactive`).
...
- **Create**: `#btn-ai-create` opens the graph-side AI Create popup (`#graph-ai-create-dialog`) and is auto-hidden when AJpC Tools AI is not active.
...
- **Right-click** a node for actions:
  ...
  - AI Enrich... (shown only when AJpC Tools AI API is available; opens graph-side Enrich popup prefilled with the clicked note NID)
  ...
  - Context menu icons are SVG assets (`web/assets/ctx-icons/*.svg`) with per-entry hardcoded `iconSpec` (`key[:mode[:color]]`); if mode/color are omitted they default to `fixed` + `var(--text-main)`.
...
- CI workflow:
  - `.github/workflows/smoke-checks.yml` runs the same suite on `push/pull_request` for `main` and `dev`.
- Local pre-commit hook:
  - `git config core.hooksPath .githooks`
  - hook file: `.githooks/pre-commit` (runs `python scripts/check_graph_smoke_suite.py`).
